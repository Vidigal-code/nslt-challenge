...
import { Order } from '../../../order/interfaces/schemas/order.schema';
import { Product } from '../../../product/interfaces/schemas/product.schema';
    ...
    private async buildPipeline(filters: any): Promise<PipelineStage[]> {
        ...
        if (filters.productId) {
            if (!Types.ObjectId.isValid(filters.productId)) {
                throw new BadRequestException(Errors.INVALID_PRODUCT_ID);
            }
        ...
        if (filters.categoryId) {
            if (!Types.ObjectId.isValid(filters.categoryId)) {
                throw new BadRequestException(Errors.INVALID_CATEGORY_ID);
            }
